...
def savitzky_golay(y, window_size, order, deriv=0, rate=1):
    ...
    b = np.mat([[k ** i for i in order_range] for k in range(-half_window, half_window + 1)], dtype='float')
...
class Dataset(object):
    def __init__(self, data, do_smoothing, do_norm, window_size, step_size, smoothing_order=3):
        """
    ...
    #@staticmethod
    def __smoothing__(self, data):
        """
        applies smoothing to the window
        :param data: window to process
        :return: smoothed window
        """
        ret = savitzky_golay(data, self.window_size, self.smoothing_order)
        return ret
